...
void Flow::InitializeQaHistograms()
{
    ...
    h2dQa.at(kM2VsP) = new TH1F( "m^{2} vs p", ";p, #frac{GeV}{c};m^{2}, #frac{GeV^{2}}{c^{4}}", 100, -3.0, 3.0, 100, 0, 18.0 );
    h2dQa.at(kPtVsY) = new TH1F( "pt vs y_{cm}", ";y_{cm};pt, #frac{GeV}{c}", 100, -1.0, 1.0, 100, 0, 3.0 );
    h2dQa.at(kFwAdcVsEstimator) = new TH1F( "FW-ADC vs TOF+RPC-Hits", ";TOF+RPC-Hits;FW-ADC", 100, 0.0, 8000.0, 250, 0., 250.0 );
    h2dQa.at(kFwZVsEstimator) = new TH1F( "FW-Z vs TOF+RPC-Hits", ";TOF+RPC-Hits;FW-Z", 100, 0.0, 100.0, 250, 0., 250.0 );
    h2dQa.at(kFwAdcVsModuleId) = new TH1F( "FW-Module Id vs FW-ADC", ";FW-ADC;FW-Module Id", 100, 0.0, 1000.0, 304, 0., 304.0 );
    h2dQa.at(kFwZVsModuleId) = new TH1F( "FW-Module Id vs FW-Z", ";FW-Z;FW-Module Id", 20, 0.0, 20.0, 304, 0., 304.0 );
}

void Flow::FillQaHistograms()
{
    h1dQa.at(kCentrality)->Fill( fEvent->GetCentrality(HADES_constants::kNhitsTOF_RPC_cut) );
    h1dQa.at(kCentralityEsimator)->Fill( fEvent->GetCentralityEstimator(HADES_constants::kNhitsTOF_RPC_cut) );
    ...
    for(int i=0; i<nTracks; i++)
    {
        ...
        auto tof = fEvent->GetTOFHit()->GetTime();
		auto charge = fEvent->GetTOFHit()->GetCharge(); 
		auto len = fEvent->GetTOFHit()->GetPathLength();
    ...
    for(int i=0; i<nFwModules; i++)
    {
        ...
        sumZ+=fEvent->GetPSDModule(i)->GetEnergy();
    }
